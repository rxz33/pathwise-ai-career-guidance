...
class CrossExamAgent:
    def __init__(self, provider: str = "groq"):
        self.provider = provider
        self.previous_rounds: Dict[str, Dict[str, Any]] = {}   # store intro + questions

    async def call_llm(self, prompt: str) -> str:
        return await call_llm(self.provider, prompt)

    async def generate_questions(self, user_data: dict) -> Dict[str, Any]:
        """
        Generate personalized intro + 5–8 cross-exam questions.
        """
        user = UserData(**user_data)
        email = user.email or "anonymous"
...
Return your entire response ONLY as a JSON object with two keys:
- "intro": a short, comforting, personalized message using the user's name.
- "questions": a JSON array of 5–8 deeply personalized cross-examination questions.

Do NOT output anything outside the JSON object.

Use ONLY the user's real data. Ignore empty or missing fields.
...
RULES:
1. "intro" must be a warm message using the user's name naturally.
2. "questions" must contain ONLY a list of short, emotional, personalized questions.
3. Each question must combine 2–3 traits (e.g. interest + finances + weakness).
4. Include 1–2 empathetic phrases like:
   - "It's okay if this feels confusing…"
   - "Take your time with this…"
5. Make questions conversational, human, and practical.
...
"""
    ...
    async def analyze_answers(self, user_data: dict, answers: List[str]) -> Dict[str, Any]:
        """
        Analyze user answers & return updated strengths/weaknesses/suggestions.
        """
        user = UserData(**user_data)
        email = user.email or "anonymous"

        previous_data = self.previous_rounds.get(email, {})
        previous_questions = previous_data.get("questions", [])

        # -------------------- PROMPT --------------------
        prompt = f"""
You are a career counselor. Analyze the user's answers with emotional intelligence.
...
- suggestions
- next_steps
- friendly_summary (kind, short insight for the user)
"""
        # -------------------------------------------------
